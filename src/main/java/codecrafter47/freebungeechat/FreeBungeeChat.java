...
public class FreeBungeeChat extends Plugin implements Listener {
    ...
    public void sendGlobalConsoleChatMessage(String message) {
        try {
            ...
            text = text.replaceAll("%(server|group|prefix(color)?|suffix|balance|currency|currencyPl|tabName|displayName|world|health|level|killScorePrefix|BungeePerms_(Prefix|Suffix|Group))%", "");
            text = text.replace("%message%", message);
